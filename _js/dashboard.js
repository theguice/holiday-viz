...
function initDashboard(start, end, userIds) {
	...
	drawBarChart(summary, '#city-by-distance', 'overall-user-city-by-distance', 'bar', "City By Distance");
	...
	drawBarChart(summary, '#city-by-time', 'overall-user-city-by-time', 'bar', "City By Time");
	// console.log(summary)
	summary = runCustomQuery("SELECT b.user_id,b.first_name as name,  count(distinct a.city) as value from gpx_track a, gpx_users b  where a.user_id = b.user_id group by b.first_name order by count(distinct city) desc LIMIT " + LIMIT)
	// console.log("3rd barchart", summary);
	drawBarChart(summary, '#users-by-city-count', 'overall-user-users-by-city-count', 'bar', "Users by city count");

	summary = runCustomQuery("SELECT b.user_id,b.first_name as name,  round(sum(a.distance) * 0.000621371)  as value from gpx_track a, gpx_users b  where a.user_id = b.user_id group by b.first_name order by sum(a.distance) desc LIMIT " + LIMIT)
	drawBarChart(summary, '#users-by-distance', 'overall-user-users-by-distance', 'bar', "Users by Distance");
	...
	drawBarChart(summary, '#users-by-time', 'overall-user-users-by-time', 'bar', "Users by time");
